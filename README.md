...
一个轻量级的系统托盘应用，提供四大核心功能：Markdown 转图片、AI 语法修正、AI 翻译、AI 解释。选中文字按快捷键即可使用。

## ✨ 特性

- 🖼️ **Markdown 转图片** - `Option+Shift+M` 将剪贴板 Markdown 渲染为精美图片
- ✏️ **AI 语法修正** - `Option+Shift+G` 选中文字一键修正语法，附带翻译对照
- 🌐 **AI 翻译** - `Option+Shift+T` 选中文字一键中英互译
- 💡 **AI 解释** - `Option+Shift+E` 选中文字一键解释含义
...
#### 💡 AI 解释
1. 用鼠标**选中**需要解释的词语、短语或句子
2. 按下 `Option+Shift+E`
3. 弹出对话框显示 AI 对选中内容的解释
4. 可点击「复制解释」将解释内容复制到剪贴板

### 快捷键一览

| 快捷键 | 功能 | 说明 |
|--------|------|------|
| `Option+Shift+M` | Markdown 转图片 | 将剪贴板内容转为图片 |
| `Option+Shift+G` | AI 语法修正 | 修正选中文字的语法，带翻译对照 |
| `Option+Shift+T` | AI 翻译 | 中英互译选中的文字 |
| `Option+Shift+E` | AI 解释 | 解释选中文字的含义 |

> Windows/Linux 用户请将 `Option` 替换为 `Alt`
...
```javascript
const shortcut = 'Alt+Shift+M';  // 改成你想要的快捷键
```
...
├── preload-explain.js       # 预加载脚本（解释对话框）
